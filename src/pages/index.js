...
class RootIndex extends React.Component {
  render() {
    ...
    return ( <
      ...
      SEO pagePath = { `` }
      postNode = { postNode }
      pageSEO siteMetadata = { this.props.data.site.siteMetadata }
      /> <
      ...
      p >
      Our lines represent a large spectrum of possibilities.Loop provides solutions
      for Healthcare, Hospitality, Corporate, Civic,
      Cultural, Education, Retail, Residential, Liturgical, or any other scenario your project brings to the table. <
      /p> <
      /div> <
      /section> <
      section className = "section" >
      ...
      efforts at every waypoint from concept to completion– on every project from tiny to tremendous. <
      /p> <
      /div> <
      /section> <
      ...
      div className = "columns is-multiline is-variable is-8 is-centered" > {
        manufacturers.map(({ node }) => {
          return ( <
            ...
            <
            Img key = { node.slug }
            sizes = { node.logoImageDark.sizes }
            /> <
            /Link> <
            /div>
          )
